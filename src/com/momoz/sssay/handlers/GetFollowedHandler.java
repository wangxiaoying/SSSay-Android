...
public class GetFollowedHandler implements CallAPIHandlerInterface {

	private final static String TAG = "GetFollowedHandler";
	...
	public GetFollowedHandler(Context context, Handler handler, ListView lv, String token) {
		mContext = context;
		mHandler = handler;
		mLv = lv;
		user_token = token;
	...
	public void handleResponse(String str) {
		...
		try{
			...
			SimpleAdapter adapter = new SimpleAdapter(mContext, list, R.layout.list_item_followed, 
					new String[]{"name", "id", "temp_image"}, 
					new int[]{R.id.followed_name, R.id.followed_id, R.id.followed_avatar});
			...
			mLv.post(new Runnable(){
				...
				public void run() {
					...
					for (int i=0; i<c; ++i) {
						...
						ImageView portrait = (ImageView) v.findViewById(R.id.followed_avatar);
